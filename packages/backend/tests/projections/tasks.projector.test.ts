...
describe('Tasks projector', () => {
  it('TaskCreated inserts task with status ready', async () => {
    await tasksProjector({ id: 1, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskCreated', payload: { id: TASK_ID, name: 'Oil change', categoryId: CAT_ID }, version: 1, createdAt: new Date() }, pool);
  ...
  it('TaskStarted sets status to ongoing', async () => {
    await tasksProjector({ id: 1, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskCreated', payload: { id: TASK_ID, name: 'Oil change', categoryId: CAT_ID }, version: 1, createdAt: new Date() }, pool);
    await tasksProjector({ id: 2, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskStarted', payload: { id: TASK_ID }, version: 2, createdAt: new Date() }, pool);
  ...
  it('TaskCompleted sets status to done for non-recurring', async () => {
    await tasksProjector({ id: 1, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskCreated', payload: { id: TASK_ID, name: 'Oil change', categoryId: CAT_ID }, version: 1, createdAt: new Date() }, pool);
    await tasksProjector({ id: 2, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskCompleted', payload: { id: TASK_ID, itemDisposals: [] }, version: 2, createdAt: new Date() }, pool);
    const row = await pool.query('SELECT status FROM tasks_view WHERE id = $1', [TASK_ID]);
  ...
  it('TaskRescheduled resets task to planned with new due date', async () => {
    ...
    await tasksProjector({ id: 3, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskRescheduled', payload: { id: TASK_ID, nextDueDate }, version: 3, createdAt: new Date() }, pool);
  ...
  it('ItemRequirementAdded inserts into task_items_view and sets task to pending', async () => {
    ...
    await tasksProjector({ id: 1, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskCreated', payload: { id: TASK_ID, name: 'Set up solar light', categoryId: CAT_ID }, version: 1, createdAt: new Date() }, pool);
    await tasksProjector({ id: 2, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'ItemRequirementAdded', payload: { taskId: TASK_ID, itemId: ITEM_ID, consumable: true }, version: 2, createdAt: new Date() }, pool);
    const row = await pool.query('SELECT status FROM tasks_view WHERE id = $1', [TASK_ID]);
