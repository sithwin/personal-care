...
describe('PauseProjectHandler', () => {
  it('throws Project not found when history is empty', async () => {
    ...
    await expect(new PauseProjectHandler(store).handle({ type: 'PauseProjectCommand', payload: { id: 'proj-1' } }, ctx)).rejects.toThrow('Project not found');
  });
  ...
  it('throws Project is not active when status is draft', async () => {
    ...
    await expect(new PauseProjectHandler(store).handle({ type: 'PauseProjectCommand', payload: { id: 'proj-1' } }, ctx)).rejects.toThrow('Project is not active');
  });
  ...
  it('appends ProjectPaused when active', async () => {
    ...
    const result = await new PauseProjectHandler(store).handle({ type: 'PauseProjectCommand', payload: { id: 'proj-1' } }, ctx);
