...
describe('ResumeProjectHandler', () => {
  it('throws Project not found when history is empty', async () => {
    ...
    await expect(new ResumeProjectHandler(store).handle({ type: 'ResumeProjectCommand', payload: { id: 'p1' } }, ctx)).rejects.toThrow('Project not found');
  });
  ...
  it('throws Project is not on hold when status is draft', async () => {
    ...
    await expect(new ResumeProjectHandler(store).handle({ type: 'ResumeProjectCommand', payload: { id: 'p1' } }, ctx)).rejects.toThrow('Project is not on hold');
  });
  ...
  it('appends ProjectResumed when on_hold', async () => {
    ...
    await new ResumeProjectHandler(store).handle({ type: 'ResumeProjectCommand', payload: { id: 'p1' } }, ctx);
