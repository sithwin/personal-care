...
import { ScheduleTaskHandler } from './ScheduleTaskHandler';
...
describe('ScheduleTaskHandler', () => {
  it('throws Task not found when getEvents resolves empty history', async () => {
    ...
    const handler = new ScheduleTaskHandler(mockEventStore);

    await expect(handler.handle(cmd)).rejects.toThrow('Task not found');
    expect(mockEventStore.append).not.toHaveBeenCalled();
  });

  it('appends the event from aggregate.schedule with expectedVersion equal to history.length', async () => {
    const cmd: ScheduleTaskCommand = {
      type: 'ScheduleTaskCommand',
      payload: {
        id: 'task-1',
    ...
    const mockStoredEvents: StoredEvent[] = [
      {
        ...
        payload: { id: 'task-1', scheduledDate: '2026-06-20', scheduledStartTime: '09:00' },
        version: 2,
        createdAt: new Date(),
      },
    ];
    ...
    const customStoredEvents: StoredEvent[] = [
      {
        ...
        aggregateType: 'task',
        eventType: 'TaskScheduled',
        payload: { id: 'task-2', scheduledDate: '2026-06-25', scheduledStartTime: '14:30' },
        version: 2,
