...
import { DetachResourceFromTaskHandler } from './DetachResourceFromTaskHandler';
...
describe('DetachResourceFromTaskHandler', () => {
  it('throws Task not found when getEvents resolves empty history', async () => {
    const cmd: DetachResourceFromTaskCommand = {
      type: 'DetachResourceFromTaskCommand',
      payload: {
        taskId: 'task-1',
        resourceId: 'res-1',
      },
    ...
    const handler = new DetachResourceFromTaskHandler(mockEventStore);

    await expect(handler.handle(cmd, ctx)).rejects.toThrow('Task not found');
    expect(mockEventStore.append).not.toHaveBeenCalled();
  });

  it('appends the event from aggregate.detachResource with expectedVersion equal to history.length', async () => {
    const cmd: DetachResourceFromTaskCommand = {
      type: 'DetachResourceFromTaskCommand',
      payload: {
        taskId: 'task-1',
    ...
    const mockStoredEvents: StoredEvent[] = [
      {
        ...
        aggregateId: 'task-1',
        aggregateType: 'task',
        eventType: 'ResourceDetachedFromTask',
    ...
    expect(events[0].eventType).toBe('ResourceDetachedFromTask');
