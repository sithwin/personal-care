...
import { PromoteToProjectHandler } from './PromoteToProjectHandler';
...
describe('PromoteToProjectHandler', () => {
  it('throws Task not found when getEvents resolves empty history', async () => {
    ...
    const handler = new PromoteToProjectHandler(mockEventStore);

    await expect(handler.handle(cmd, ctx)).rejects.toThrow('Task not found');
    expect(mockEventStore.append).not.toHaveBeenCalled();
  });

  it('appends the event from aggregate.promoteToProject with expectedVersion equal to history.length', async () => {
    const cmd: PromoteToProjectCommand = {
      type: 'PromoteToProjectCommand',
      payload: {
        taskId: 'task-1',
    ...
    const mockStoredEvents: StoredEvent[] = [
      {
        ...
        aggregateId: 'task-1',
        aggregateType: 'task',
        eventType: 'TaskPromotedToProject',
    ...
    const customStoredEvents: StoredEvent[] = [
      {
        ...
        aggregateType: 'task',
        eventType: 'TaskPromotedToProject',
        payload: { taskId: 'task-2', projectId: 'proj-2' },
        version: 2,
