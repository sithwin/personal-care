...
describe('Items projector', () => {
  it('ItemCreated inserts item with status to_buy', async () => {
    await itemsProjector({ id: 1, aggregateId: ITEM_ID, aggregateType: 'item', eventType: 'ItemCreated', payload: { id: ITEM_ID, name: 'Solar light', categoryId: CAT_ID }, version: 1, createdAt: new Date() }, pool);
  ...
  it('MarkItemAvailable updates item and unblocks tasks', async () => {
    await itemsProjector({ id: 1, aggregateId: ITEM_ID, aggregateType: 'item', eventType: 'ItemCreated', payload: { id: ITEM_ID, name: 'Solar light', categoryId: CAT_ID }, version: 1, createdAt: new Date() }, pool);
    await tasksProjector({ id: 2, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'TaskCreated', payload: { id: TASK_ID, name: 'Set up solar light', categoryId: CAT_ID }, version: 1, createdAt: new Date() }, pool);
    await tasksProjector({ id: 3, aggregateId: TASK_ID, aggregateType: 'task', eventType: 'ItemRequirementAdded', payload: { taskId: TASK_ID, itemId: ITEM_ID, consumable: true }, version: 2, createdAt: new Date() }, pool);
    const before = await pool.query('SELECT status FROM tasks_view WHERE id = $1', [TASK_ID]);
    expect(before.rows[0].status).toBe('pending');
    await itemsProjector({ id: 4, aggregateId: ITEM_ID, aggregateType: 'item', eventType: 'ItemMarkedAvailable', payload: { id: ITEM_ID }, version: 2, createdAt: new Date() }, pool);
