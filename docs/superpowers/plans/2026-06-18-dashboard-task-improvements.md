# Dashboard Task Improvements Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the Dashboard's CommandBar search-only, add an inline "+ Add Task" form to the Up Next section, and make Up Next task rows actionable (start/complete + navigate to full task).

**Architecture:** Two focused frontend changes — strip task creation from CommandBar and replace with `useNavigate`-based search; extend Dashboard with a local `NewTaskRow` component and interactive Up Next rows. No backend changes, no new routes.

**Tech Stack:** React 18, TypeScript, Tailwind CSS, TanStack Query, react-router-dom v6, vitest (no `@testing-library/react` installed — verification is manual via dev server)
...
- Never `any` — use proper types throughout
- No inline try/catch — dispatch errors propagate naturally
- Follow existing Tailwind class patterns in adjacent components

---

### Task 1: CommandBar — search-only

**Files:**
- Modify: `packages/frontend/src/components/layout/CommandBar.tsx`

**Interfaces:**
- Consumes: `useNavigate` from `react-router-dom`
- Produces: unchanged export `CommandBar` — same props signature, different behaviour

- [ ] **Step 1: Replace the file contents**

Replace `packages/frontend/src/components/layout/CommandBar.tsx` with:
...
Run: `npm run dev:frontend` (backend must be running for queries to succeed, but not required for this UI check)

Check:
1. Click the CommandBar on the dashboard — modal opens with a single text input, no category select, no "Create Task" button
2. Type a word and click "Search" — navigates to `/tasks?search=<word>` (Tasks page loads, search param in URL)
3. Press `Ctrl+K` / `Cmd+K` — modal opens
4. Press `Escape` — modal closes

- [ ] **Step 4: Commit**

```bash
git add packages/frontend/src/components/layout/CommandBar.tsx
git commit -m "refactor(frontend): make CommandBar search-only, remove task creation"
```

---

### Task 2: Dashboard — inline Add Task form and actionable Up Next rows

**Files:**
- Modify: `packages/frontend/src/pages/Dashboard.tsx`

**Interfaces:**
- Consumes: `dispatch` from `../api/commands`; `useCategories`, `useDashboard`, `useProjects` from `../api/queries`; `useQueryClient` from `@tanstack/react-query`; `v4 as uuidv4` from `uuid`; `Link` from `react-router-dom`
- Produces: unchanged export `Dashboard`

- [ ] **Step 1: Replace the file contents**

Replace `packages/frontend/src/pages/Dashboard.tsx` with:
...
Run: `npm run dev:frontend` (requires backend + docker postgres running for live data)

Check the following in the browser at `http://localhost:5173`:

**Up Next — "+ Add Task" button:**
1. The Up Next section header shows a `+ Add Task` button on the right
2. Clicking it reveals an inline form with a name input (auto-focused) and category select
3. Typing a name and clicking "Add task" creates the task and dismisses the form; the new task appears in Up Next
4. Clicking "Cancel" dismisses the form without creating anything
5. The `+ Add Task` button is hidden while the form is open

**Up Next — task row actions:**
1. For a task with `status === 'ready'`: clicking the `☐` checkbox dispatches `StartTaskCommand` — the task moves to `ongoing` and disappears from Up Next (or shows as ongoing if that status is also shown)
2. For a task with `status === 'ongoing'`: clicking the `☐` checkbox dispatches `CompleteTaskCommand` — the task moves to `done`
3. The task name is a link; clicking it navigates to `/tasks?status=<status>`

**Up Next — empty state:**
1. When `upNext` is empty and `addingTask` is false, the section shows "No tasks up next." with the `+ Add Task` button still visible
2. Clicking `+ Add Task` in the empty state opens the inline form correctly

- [ ] **Step 4: Commit**

```bash
git add packages/frontend/src/pages/Dashboard.tsx
git commit -m "feat(frontend): add inline task creation and actionable rows to Up Next"
```
